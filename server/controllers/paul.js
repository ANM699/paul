...
async function paulTop(ctx) {
  ...
  let paulTop = await mysql.raw('SELECT COUNT(paul_id) AS paul_ct,(paul_home+paul_away) AS paul_sum, paul_home,paul_away FROM paul WHERE match_id=' + matchId + ' GROUP BY paul_home,paul_away ORDER BY paul_ct DESC,paul_sum ASC,paul_home ASC,paul_away ASC LIMIT 10')
