...
async function retrieve(ctx, next) {
    if (ctx.query.competitionId) {
        ...
        if (parseInt(ctx.query.down)) { //向下查询15条
            ctx.state.data = await mysql('matchs').where({ competition_id: competitionId, enable: 1 }).andWhere('match_time', '>=', startDate).orderByRaw('match_time ASC,round ASC').limit(15)
        } else { //向上查询15条
            ctx.state.data = await mysql('matchs').where({ competition_id: competitionId, enable: 1 }).andWhere('match_time', '<=', startDate).orderByRaw('match_time DESC,round DESC').limit(15)
        }
...
async function update(ctx, next) {
    ...
    await mysql.transaction(async function(trx) {
        ...
        if (match.score_home && match.score_away) {
            ...
            await trx('paul').update({ paul_score: null }).whereRaw('match_id=?', [match_id])
        }
    })
}
